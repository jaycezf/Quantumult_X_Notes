...
function getAppSign(t) {
    const sign = 'f=android&sk=1&time=' + t + '&token=' + getToken() +
...
function getBody() {
    ...
    return 'touchstone_event=&v=10.0&sign=' + sign + '&weixin=0&time=' + t + '&sk=1&token=' + token +
        '&f=android&captcha='
}
function showmsg() {
    return new Promise((resolve) => {
        ...
        if ($.web.error_code === 0 && $.web.data) {
            $.desc.push(
                `累计: ${$.web.data.checkin_num}次, 经验: ${$.web.data.exp}, 金币: ${$.web.data.gold}, 积分: ${$.web.data.point}`
            )
        }
        if ($.app) {
...
function Env(t, e) {
    ...
    return new class {
        ...
        post(t, e = (() => {})) {
            ...
            else if (this.isQuanX()) t.method = s, this.isNeedRewrite && (t.opts = t.opts || {}, Object.assign(
                t.opts, {
                    hints: !1
                })), $task.fetch(t).then(t => {
                const {
